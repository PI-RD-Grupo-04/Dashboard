				...
				<nav aria-label="breadcrumb">
					<ol
						...
						<li class="breadcrumb-item text-sm text-dark active"
							aria-current="page">cadastro tipo de frete</li>
					</ol>
					<h1 class="-bolder mb-0">cadastro de tipo de frete</h1>
				</nav>
		...
		<div class="container-fluid ">
			<div class="row">
				<div class="col-md-5 mx-auto border text-center">
					<c:choose>
						<c:when test="${tipoFrete == null}">
							<h3>Cadastrar tipo de frete</h3>
						</c:when>
						<c:otherwise>
							<h3>Atualizar tipo de frete</h3>
						</c:otherwise>
					</c:choose>
					<form method="post" action="ServletTipoFrete">
						<input type="hidden" name="id" value="${tipoFrete.id}" />
						...
						<div class="form-group" style="text-align-last: left">
							<label for="nome" class="form-label">Tipo de Frete:</label> <input
								type="text" class="form-control border "
								style="background-color: rgb(255, 255, 255);" name="descricao_frete"
								value="${tipoFrete.descricao_frete}" required>
						</div>
						<div class="d-grid gap-2">
							<c:choose>
								<c:when test="${tipoFrete == null}">
